...
### Fixed
- **Uzbekistan/Tashkent Tickets Only Tagged One of Two Responsibles**: `data/responsibles.json` on the production server is a bind-mounted volume (`docker-compose.yml`), and the deploy pipeline (`.gitlab-ci.yml`/`.github/workflows/deploy.yml`) runs `git reset --hard origin/main` on every deploy — which silently overwrites the file with whatever is actually committed to `origin/main`, discarding any local-only edit. A previous addition of `Artyom Ashminevich` as a second responsible for `uzbekistan`/`tashkent` (alongside `Bogdan Martemyanov`) had only ever been made in an uncommitted working copy, so every deploy kept resetting the file back to the single-responsible version — `_build_mention_entities()` (`bot/teams.py`) was working correctly, it simply never had Artyom in the data it read. Committed and pushed the missing entry to `origin/main`. **Takeaway**: any edit to `data/responsibles.json` (or any file under `data/`) must be committed and pushed to `origin main`, or it will not survive the next deploy.
